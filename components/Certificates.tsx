      ...
      <div className="w-full border-b border-gray-300">
        {items.map((it, idx) => (
          <a
            ...
            className="group flex flex-col md:flex-row items-start md:items-center justify-between py-12 md:py-20 px-8 w-full bg-transparent transition-colors border-t border-gray-300 first:border-t-0 overflow-hidden"
            onMouseEnter={() => setHoveredId(it.id)}
            onMouseLeave={() => setHoveredId(null)}
          >
            <div className="flex flex-col md:flex-row md:items-center gap-4 w-full md:w-auto z-10 pointer-events-none">
              <div className="w-6 pr-10 text-gray-400">0{idx + 1}</div>
              <div
                className={`text-xl md:text-2xl font-medium text-${it.textcolor} group-hover:translate-x-4 transition-transform duration-500 page-specific-font`}
              >
                {it.title}
              </div>
            </div>
            ...
            <div
              className={`flex items-center gap-3 mt-4 md:mt-0 text-${it.textcolor} z-10 pointer-events-none page-specific-font`}
            >
              <div>{it.date}</div>
